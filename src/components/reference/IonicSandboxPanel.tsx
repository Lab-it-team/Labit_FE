...
function snapDroppedPiece(pieces: PlacedPiece[], dropId: string, dropX: number, dropY: number): PlacedPiece[] {
...
export default function IonicSandboxPanel() {
  ...
  const mkId = () => `rp${idCounter.current++}`;
  ...
  const { data: compoundInfo, isFetching, isError } = useQuery({
  ...
  const handleSave = () => {
    ...
    setCompleted((prev) => [...prev, { formula: currentFormula, name_ko: compoundInfo?.name_ko ?? null }]);
    setPlacedPieces([]);
  };
  ...
  function renderCompoundBody() {
    if (!isBalanced) {
      return (
        <>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 32, lineHeight: "40px", color: "var(--color-text-disabled)" }}>—</span>
    ...
    if (isFetching) {
      return (
        <>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 32, lineHeight: "40px", color: "var(--color-text-disabled)" }}>{currentFormula}</span>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 13, lineHeight: "18px", color: "var(--color-text-disabled)" }}>확인 중...</span>
        </>
      );
    }
    if (isError) {
      return (
        <>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 32, lineHeight: "40px", color: "var(--color-text-disabled)" }}>{currentFormula}</span>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 12, lineHeight: "18px", color: "var(--color-text-sub)", textAlign: "center" }}>정보를 불러올 수 없어요</span>
        </>
      );
    }
    if (compoundInfo?.exists) {
      ...
      return (
        <>
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 32, lineHeight: "40px", color: "var(--color-primary-normal)" }}>{currentFormula}</span>
          {compoundInfo.name_ko && (
            <span style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 14, lineHeight: "20px", color: "var(--color-text-normal)" }}>{compoundInfo.name_ko}</span>
          )}
          <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
            ...
            <span style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 12, lineHeight: "18px", color: "#17B26A" }}>이온 화합물이에요</span>
          </div>
          {(infoRows.length > 0 || compoundInfo.notes) && (
            <div style={{ width: "100%", borderTop: "1px solid var(--color-border-light)", marginTop: 6, paddingTop: 8, display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start" }}>
              {infoRows.map(({ label, value }) => (
                <div key={label} style={{ display: "flex", gap: 6, width: "100%" }}>
                  <span style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 11, lineHeight: "18px", color: "var(--color-text-sub)", flexShrink: 0, minWidth: 36 }}>{label}</span>
                  <span style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 11, lineHeight: "18px", color: "var(--color-text-normal)" }}>{value}</span>
                </div>
              ))}
              {compoundInfo.notes && (
                <p style={{ margin: 0, fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 11, lineHeight: "17px", color: "var(--color-text-sub)", fontStyle: "italic", marginTop: infoRows.length > 0 ? 2 : 0 }}>
                  {compoundInfo.notes}
                </p>
              )}
            </div>
    ...
    return (
      <>
        <span style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 32, lineHeight: "40px", color: "#F07B2F" }}>{currentFormula}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <span style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 12, lineHeight: "18px", color: "#F07B2F" }}>이온 화합물이 아니에요</span>
        </div>
        {compoundInfo?.reason_if_not && (
          <div style={{ width: "100%", background: "#FFF6ED", border: "1px solid #FAD6BF", borderRadius: 8, padding: "10px 12px", marginTop: 4 }}>
            <p style={{ margin: 0, fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 12, lineHeight: "18px", color: "#B05100" }}>{compoundInfo.reason_if_not}</p>
          </div>
        )}
      </>
    );
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragCancel={handleDragCancel} onDragEnd={handleDragEnd}>
      ...
      <div style={{ display: "flex", flexDirection: "row", alignItems: "flex-start", gap: 16, width: "100%" }}>
        <IonTabList draggingIonId={draggingPaletteId} height={PALETTE_H} width={312} />
            ...
            <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 0 }}>
              <ToolBtn
                onClick={() => {
                  ...
                  setPlacedPieces((prev) => removePieceLinks(prev.slice(0, -1), lastId));
                }}
                disabled={!hasInput}
              >
                되돌리기
              </ToolBtn>
              <ToolBtn onClick={() => setPlacedPieces([])} disabled={!hasInput}>초기화</ToolBtn>
            </div>
            <button
              type="button"
              onClick={handleSave}
        ...
        <div style={{ width: SIDEBAR_W, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12, alignSelf: "stretch" }}>
          <div
            style={{
              boxSizing: "border-box",
              display: "flex",
          ...
          >
            <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 4 }}>
              <span style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 13, lineHeight: "20px", letterSpacing: "-0.005em", color: "var(--color-text-sub)" }}>
                지금까지 완성한 화합물
              </span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, overflowY: "auto", flex: 1 }}>
              {completed.length === 0 ? (
                <span style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 13, lineHeight: "18px", color: "var(--color-text-disabled)", textAlign: "center" }}>—</span>
              ) : (
                completed.map((c, i) => (
                  <div key={i} style={{ display: "flex", flexDirection: "row", alignItems: "center", padding: "6px 10px", background: "var(--color-bg-elevate)", borderRadius: 8, gap: 6 }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                      <span style={{ fontFamily: "var(--font-sans)", fontWeight: 600, fontSize: 15, lineHeight: "20px", color: "var(--color-text-strong)" }}>{c.formula}</span>
                      {c.name_ko && (
                        <span style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 12, lineHeight: "16px", color: "var(--color-text-sub)" }}>{c.name_ko}</span>
                      )}
                    </div>
                  </div>
                ))
